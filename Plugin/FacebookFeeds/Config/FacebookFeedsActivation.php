...
//			'title' => 'FacebookFeeds',
//			'link' => 'plugin:facebook_feeds/controller:example/action:index',
//			'status' => 1,
//			'class' => 'example',
//		));
	}
...
//		$controller->Croogo->removeAco('Example'); // ExampleController ACO and it's actions will be removed
		...
		$link = $this->Link->find('first', array(
			...
			'conditions' => array(
                                'Link.link' => 'plugin:facebook_feeds/controller:example/action:index',
			),
		));
		if (empty($link)) {
			return;
