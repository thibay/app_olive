...
class MediasController extends AppController{
    ...
    public function home_random($limit = 16){        
        $this->Media->recursive = 0;
        return $this->Media->find("all", array('fields'=>array('file','ref_id'), 'limit'=>$limit, 'order'=>'RAND()'));
    }
    ...
    **/
    public function index($ref,$ref_id){  
        ...
        if(!in_array('Media', $this->$refmodel->Behaviors->loaded())){
        ...
        $thumbID = false;
        if($this->$refmodel->hasField('media_id')){
            $this->$refmodel->id = $ref_id;
            $thumbID = $this->$refmodel->field('media_id');
        }
        $extensions = $this->$refmodel->medias['extensions'];
    ...
    **/
    public function upload($ref,$ref_id){        
        ...
        $media = $media['Media'];
        $thumbID = $this->$refmodel->hasField('media_id');
        $editor = isset($this->request->params['named']['editor']) ? $this->request->params['named']['editor'] : false;
        $id = isset($this->request->query['id']) ? $this->request->query['id'] : false;
    ...
    **/
    public function thumb($id){
        ...
        $refmodel = end(explode('.', $ref));                
        $this->$refmodel->id = $ref_id;
        $this->$refmodel->saveField('media_id',$id);
